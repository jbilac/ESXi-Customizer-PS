...
if ($help) {
    write-host "`nUsage:"
    write-host "  ESXi-Customizer-PS [-help] |  [-izip <bundle> [-update]] [-sip] [-v80|-v70|-v67|-v65|-v60|-v55|-v51|-v50]"
...
if ($iZip -and !($update)) {
    ...
    if ($v80) {
        ...
        if (( $pcmv.Major -le 12 ) -and ( $pcmv.Minor -le 7 )) {
            ...
            Get-EsxImageprofile "ESXi-8.0*" -Softwaredepot $basedepot | foreach { $iplist += $_ }
        }
    } elseif ($v70) {
